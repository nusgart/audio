...
struct Source {
  ...
  void setData(Sound s, float x, float y, float z, float vx, float vy, float vz,
  ...
  void dispose() { alDeleteSources(1, &sourceID); }
};
...
Sound loadPCMSound(FILE *soundFile) {
  ...
  s.frequency = data[2];
  s.length = data[3];
  cerr << "Magic: " << data[0] << " frequency: " << s.frequency
...
int main(int argc, char *argv[]) {
  ...
  device = alcOpenDevice(
      default_device); // alcOpenDevice(NULL, ALC_DEFAULT_DEVICE_SPECIFIER);
  if (device == NULL) {
    // Handle Exception
  ...
  cerr << "Num Sources: " << numSources << " num mono: " << mono
       << " num stereo: " << stereo << endl;
  cerr << "AL error: " << alGetError() << endl;
  for (int i = 0; i < numSources; i++) {
    // generate a source
  ...
  while (!quit) {
    ...
    if (in.find("load") != string::npos) {
      ...
      string fname;
      cin >> name >> fname;
      cerr << "Loading  sound file " << fname << " as " << name << endl;
      bool ret = loadSound(name, fname);
      if (!ret) {
        cout << "ERROR 2: Sound file does not exist or is not valid file"
             << endl;
      } else {
